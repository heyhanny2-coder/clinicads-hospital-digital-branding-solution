...
const Portfolio: React.FC = () => {
  ...
  return (
    <div className="pt-32 pb-24 bg-brand-navy">
      <div className="max-w-6xl mx-auto px-6">
        <div className="mb-16 text-center">
          <p className="text-[10px] font-bold text-brand-lavender tracking-[0.25em] uppercase mb-4">Case Studies</p>
          <h1 className="text-2xl md:text-3xl font-black text-white mb-6">결과로 증명합니다.</h1>
          <p className="text-white/80 text-sm font-normal leading-relaxed max-w-xl mx-auto">
                  ...
                  {pageItems.map((item) => (
                    <div key={item.id} className="flex-1 min-w-0">
                      <div className="group relative bg-white/[0.06] border border-white/15 rounded-2xl p-6 hover:border-blue-400/40 transition-all duration-300 flex flex-col min-h-[220px] justify-between h-full">
                        <div className="absolute top-0 left-0 w-0.5 h-full bg-brand-lavender/30 group-hover:bg-blue-400 rounded-l-2xl" />
                        <div>
                          <div className="flex justify-between items-start mb-4">
                            <span className="text-brand-lavender text-[9px] font-bold tracking-[0.15em] uppercase bg-brand-lavender/15 px-2.5 py-0.5 rounded-full">
                              {item.category}
                            </span>
                            <span className="text-white/70 text-[9px] font-semibold tracking-[0.08em] uppercase">
                              {item.location}
                            </span>
                          </div>

                          <h3 className="text-lg font-bold text-white group-hover:text-brand-lavender transition-colors mb-5 leading-tight">
                            {item.title}
                          </h3>

                          {item.review && (
                            <div className="relative pt-4 border-t border-white/10">
                              <Quote className="absolute -top-2 left-0 text-brand-lavender/25 w-5 h-5" />
                              <p className="text-white/80 text-sm leading-relaxed italic font-normal">
                                "{item.review}"
                              </p>
                            </div>
                          )}
                        ...
                        <div className="mt-6 flex items-center gap-2 text-[9px] font-bold tracking-widest text-white/70 uppercase group-hover:text-brand-lavender transition-colors">
                          <span>View Case Study</span>
                          <div className="w-6 h-[1px] bg-brand-navy/60 group-hover:bg-blue-400 transition-all" />
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {pages.map((_, i) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all ${
                i === clampedIndex ? 'bg-brand-lavender w-6' : 'bg-brand-navy/60 hover:bg-brand-navy/50'
              }`}
        ...
        <div className="mt-20">
          <p className="text-white/70 italic font-serif text-base text-center mb-8">
          ...
          <div className="overflow-hidden marquee-container">
            <div className="flex animate-marquee w-max">
              {[...PORTFOLIO, ...PORTFOLIO].map((p, i) => (
                <span
                  key={`${p.id}-${i}`}
                  className="flex-shrink-0 text-sm font-bold italic whitespace-nowrap text-white/70/70 px-8"
                >
                  {p.title}
                </span>
              ))}
